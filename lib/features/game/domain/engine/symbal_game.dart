...
class SymbalGame extends FlameGame with HasKeyboardHandlerComponents, HasCollisionDetection {
...
SymbalGame(){
  ...
  Future<void> initializeGame(GameData data) async {
    gameData = data;
...
class SymbalEntity extends PositionComponent with CollisionCallbacks, HasGameReference<SymbalGame> {
  ...
  void _applyCollectibleEffect(CollectibleEffect effect) {
    switch (effect.type) {
      case EffectType.health:
        health = math.min(health + effect.value, entityData.components.health?.max.toDouble() ?? health);
...
class AssetManager {
  ...
  Future<SpriteAnimation?> getSpriteAnimation(String assetName, int frameCount, double stepTime) async {
...
class MovementController extends Component {
  ...
  void update(double dt) {
    ...
    // Horizontal movement
    if (leftPressed) {
      final force = entity.isGrounded ? currentSpeed : currentSpeed * (movementData.airControl ?? 1.0);
  ...
  void _updatePatrol(double dt) {
    ...
    switch (currentState) {
      ...
      case AIState.waiting:
        waitTimer -= dt;
        if (waitTimer <= 0) {
          currentWaypointIndex = (currentWaypointIndex + 1) % aiData.waypoints.length;
